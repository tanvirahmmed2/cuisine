...
export async function POST(req) {
  try {
    ...
    const { rows } = await pool.query(
      "SELECT id, verification_token_expires FROM restaurant_users WHERE verification_token = $1 LIMIT 1",
      [token]
    );
    ...
    await pool.query(
      "UPDATE restaurant_users SET is_verified = TRUE, verification_token = NULL, verification_token_expires = NULL WHERE id = $1",
      [user.id]
    );
