...
export async function GET(request, { params }) {
  try {
    ...
    // Verify ownership via ress_purchases
    const purchaseCheck = await query("SELECT purchase_id FROM ress_purchases WHERE user_id = $1 AND tenant_id = $2 LIMIT 1", [session.user_id, tenantId]);
...
export async function PATCH(request, { params }) {
  try {
    ...
    // Allowed fields for update in restaurant_websites
    const allowedFields = ['name', 'address', 'tagline', 'sociallink', 'email', 'phone', 'hero_title', 'hero_subtitle'];
    
    let queryStr = "UPDATE restaurant_websites SET ";
    const values = [];
    let count = 1;
    ...
    // Wait, restaurant_websites has tenant_id as UNIQUE constraint, so we can't easily upsert dynamically with this query builder unless we do it explicitly.
    ...
    if (updateRes.rowCount === 0) {
       ...
       await query(`INSERT INTO restaurant_websites (${insertFields}) VALUES (${insertValues})`, insertParams);
    }
