...
export async function GET() {
  try {
    ...
    const purchasesRes = await query(`
      ...
      JOIN ress_packages pk ON pk.package_id = p.package_id
      LEFT JOIN ress_tenants t ON p.tenant_id = t.tenant_id
      LEFT JOIN ress_subscriptions s ON s.tenant_id = p.tenant_id AND s.status = 'active'
      WHERE p.user_id = $1
