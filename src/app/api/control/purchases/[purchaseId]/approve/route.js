...
export async function POST(request, { params }) {
  try {
    ...
    const res = await query("SELECT * FROM ress_purchases WHERE purchase_id = $1", [purchaseId]);
      ...
      await withTransaction(async (client) => {
        ...
        const subRes = await client.query(
          `SELECT s.subscription_id, s.end_date, s.package_id, p.monthly_price 
           ...
           JOIN ress_packages p ON p.package_id = s.package_id
        ...
        const newPkgRes = await client.query("SELECT monthly_price FROM ress_packages WHERE package_id = $1", [purchase.package_id]);
        ...
        // 7. Update pending Subscription Payment Record
        await client.query(
          "UPDATE ress_subscription_payments SET subscription_id = $1, status = 'success', paid_at = NOW() WHERE purchase_id = $2",
          [subscription.subscription_id, purchaseId]
        );

        // 8. Mark purchase as paid
        await client.query(
          "UPDATE ress_purchases SET status = 'paid' WHERE purchase_id = $1",
          [purchaseId]
        );

        // 9. Update pending Invoice
        await client.query(
          "UPDATE ress_invoices SET subscription_id = $1, status = 'paid', due_date = NOW() WHERE purchase_id = $2",
          [subscription.subscription_id, purchaseId]
        );
      });
    ...
    const slugCheck = await query("SELECT tenant_id FROM ress_tenants WHERE slug = $1 LIMIT 1", [requestedTenantSlug]);
    ...
    await withTransaction(async (client) => {
      ...
      const tenantRes = await client.query(
        "INSERT INTO ress_tenants (name, slug, status) VALUES ($1, $2, 'active') RETURNING *",
      ...
      await client.query(
        "INSERT INTO ress_domains (tenant_id, domain, is_primary, verified) VALUES ($1, $2, true, true)",
      ...
      await client.query(
        "INSERT INTO restaurant_websites (tenant_id, hero_title, name, email) VALUES ($1, $2, $3, $4)", 
        [newTenant.tenant_id, requestedTenantName, requestedTenantName, tenantAdminEmail]
      );

      // 3. Create Subscription
      const subRes = await client.query(
        `INSERT INTO ress_subscriptions (tenant_id, package_id, status, start_date, end_date) 
         VALUES ($1, $2, 'active', NOW(), NOW() + INTERVAL '1 month' * $3) RETURNING *`,
        [newTenant.tenant_id, purchase.package_id, purchase.duration_months || 1]
      );
      
      // 4. Update Pending Subscription Payment Record
      await client.query(
        "UPDATE ress_subscription_payments SET subscription_id = $1, status = 'success', paid_at = NOW() WHERE purchase_id = $2",
        [subRes.rows[0].subscription_id, purchaseId]
      );


      // 6. Mark purchase as paid and link tenant
      await client.query(
        "UPDATE ress_purchases SET status = 'paid', tenant_id = $1 WHERE purchase_id = $2",
        [newTenant.tenant_id, purchaseId]
      );

      // 6.5. Update pending Invoice
      await client.query(
        "UPDATE ress_invoices SET tenant_id = $1, subscription_id = $2, status = 'paid', due_date = NOW() WHERE purchase_id = $3",
        [newTenant.tenant_id, subRes.rows[0].subscription_id, purchaseId]
      );

      // 7. Create Onboarding Support Ticket
      await client.query(
        "INSERT INTO ress_support_tickets (user_id, subject, message, priority, status) VALUES ($1, $2, $3, 'high', 'open')",
        [
          purchase.user_id, 
          'Welcome to Cuisine! Your Workspace is Active 🎉', 
          'Hello!\n\nYour purchase has been approved and your workspace has been successfully provisioned. We are thrilled to have you on board.\n\nIf you have any questions, need help setting up your custom domain, or need assistance configuring your menus, please reply directly to this ticket.\n\nBest,\nThe Cuisine Support Team'
        ]
      );
    });
