...
export async function GET(request) {
  try {
    ...
    if (contactId) {
        const res = await query("SELECT * FROM ress_contact WHERE contact_id = $1", [contactId]);
    ...
    let queryStr = "SELECT * FROM ress_contact";
    const params = [];
...
export async function POST(request) {
  try {
    ...
    const contactRes = await query("SELECT * FROM ress_contact WHERE contact_id = $1", [contact_id]);
    if (contactRes.rows.length === 0) return NextResponse.json({ success: false, message: 'Message not found' }, { status: 404 });

    const contact = contactRes.rows[0];

    // Mark as resolved
    await query("UPDATE ress_contact SET status = 'resolved' WHERE contact_id = $1", [contact_id]);
...
export async function PATCH(request) {
  try {
    ...
    await query("UPDATE ress_contact SET status = $1 WHERE contact_id = $2", [status, contact_id]);
