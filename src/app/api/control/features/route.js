...
export async function GET() {
  try {
    ...
    const res = await query("SELECT * FROM ress_features ORDER BY feature_id ASC");
...
export async function POST(request) {
  try {
    ...
    const res = await query(
      "INSERT INTO ress_features (key, name) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET name = EXCLUDED.name RETURNING *",
      [key, name]
    );
