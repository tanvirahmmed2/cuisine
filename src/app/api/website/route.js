...
export async function POST(req) {
  try {
    ...
    const { rows: existing } = await pool.query("SELECT website_id FROM websites LIMIT 1");
    ...
    if (existing.length === 0) {
      ...
      const insertQuery = `INSERT INTO websites (${insertCols}) VALUES (${insertPlaceholders}) RETURNING *`;
    ...
    const values = Object.values(updates);
    const query = `UPDATE websites SET ${setClause}, updated_at = NOW() RETURNING *`;
