...
export async function POST(request) {
  try {
    ...
    const res = await query("SELECT user_id FROM ress_users WHERE verification_token = $1", [token]);
    ...
    await query(
      "UPDATE ress_users SET is_verified = true, verification_token = NULL WHERE user_id = $1",
      [userId]
    );
