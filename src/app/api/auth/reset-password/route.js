...
export async function POST(request) {
  try {
    ...
    const res = await query(
      "SELECT user_id FROM ress_users WHERE reset_password_token = $1 AND reset_password_expires > NOW()", 
      [token]
    );
    ...
    await query(
      "UPDATE ress_users SET password = $1, reset_password_token = NULL, reset_password_expires = NULL WHERE user_id = $2",
      [hashed, userId]
    );
