        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl p-8 border border-border shadow-sm">
            ...
            {existingWorkspaces.length > 0 ? (
              <div className="p-6 bg-pink-50 border border-pink-200 rounded-2xl">
                <h3 className="text-lg font-bold text-pink-900 mb-2">You already own this package!</h3>
                <p className="text-sm text-pink-800 mb-6 leading-relaxed">
                ...
                <div className="flex flex-col gap-3">
                  {existingWorkspaces.map(w => (
                    <div key={w.tenant_id} className="flex items-center justify-between bg-white p-4 rounded-xl border border-pink-100 shadow-sm">
                      <div>
                      ...
                      <Link href={`/dashboard/workspaces/${w.tenant_id}`} className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white text-xs font-bold rounded-lg transition-colors">
                        Renew Workspace
                      </Link>
                    </div>
                  ))}
                ...
                <div className="mt-8 pt-6 border-t border-pink-200/60 text-center">
                  <p className="text-xs text-pink-700 mb-3">Or do you want to create another separate workspace on this plan?</p>
                  <button onClick={() => setExistingWorkspaces([])} className="text-sm text-pink-600 font-bold hover:underline">
