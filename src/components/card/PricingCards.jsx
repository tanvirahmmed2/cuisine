            ...
            {pkg.max_menu_items != null && (
              <li className="text-sm font-medium flex items-start gap-3">
                ...
                <span className={`${isPopular ? 'text-white/90' : 'text-text-2'}`}>
                  <strong className={isPopular ? 'text-white' : 'text-text'}>{pkg.max_menu_items === -1 ? 'Unlimited' : pkg.max_menu_items}</strong> Menu Items
                </span>
              </li>
            )}
            {pkg.max_orders_per_month != null && (
              <li className="text-sm font-medium flex items-start gap-3">
                <span className={`shrink-0 mt-0.5 ${isPopular ? 'text-white' : 'text-primary'}`}>
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                ...
                <span className={`${isPopular ? 'text-white/90' : 'text-text-2'}`}>
                  <strong className={isPopular ? 'text-white' : 'text-text'}>{pkg.max_orders_per_month === -1 ? 'Unlimited' : pkg.max_orders_per_month}</strong> Orders / month
                </span>
              </li>
            )}
            {pkg.max_tables != null && (
              <li className="text-sm font-medium flex items-start gap-3">
                ...
                <span className={`${isPopular ? 'text-white/90' : 'text-text-2'}`}>
                  <strong className={isPopular ? 'text-white' : 'text-text'}>{pkg.max_tables === -1 ? 'Unlimited' : pkg.max_tables}</strong> Tables
                </span>
              </li>
            )}
